...
- 错误模型统一：新增 `AppError → jsonError` 适配（`src/lib/http.ts`），控制器统一映射（如生成与金融模块）
- 应用层依赖注入：`GenerateCardUseCase` 通过容器 `AIProvider` 获取服务（`src/application/usecases/GenerateCardUseCase.ts`）
- 关键路径并行与降级：文案与 AI 并行执行，加入超时与重试；AI 失败时返回占位图
- 图像并发控制：用信号量替换忙等轮询，`sharp` 增加超时与像素上限
- 指标治理：引入导出器接口与采样率，默认 LoggerExporter，减少高 QPS 日志开销
- 测试增强：新增并行与占位降级、图像回退、限流回退与滑动窗口测试
