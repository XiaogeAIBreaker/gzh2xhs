...
function parseAllowedOrigins(envVal?: string): Set<string> {
  const origins = (envVal || '').split(',').map(s => s.trim()).filter(Boolean)
  return new Set(origins)
}

function isOriginAllowed(req: NextRequest, originHeader: string | null, allowed: Set<string>): boolean {
...
export function middleware(req: NextRequest) {
  ...
  if (unsafe && origin && !isAllowed) {
    return NextResponse.json({ success: false, error: 'Forbidden: CSRF origin check failed' }, { status: 403 })
  }
