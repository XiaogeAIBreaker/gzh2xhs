...
-   **位置**: `apps/api`
-   **架构**: 模块化架构 (Modules, Services, Controllers, Repositories)
-   **技术栈**: NestJS (Express), TypeORM (Postgres/SQLite), JWT Auth, Swagger
-   **状态**: 正在进行全面重构迁移，目前已完成 Auth 模块的重构与测试。
...
# 运行测试 (含覆盖率)
npm run test:jest --workspace apps/api -- --coverage
```

### 重构进度

-   [x] **基础架构**: 模块化目录结构，TypeScript 严格模式
-   [x] **认证模块**: JWT 认证，User 实体，单元测试覆盖率 > 80%
-   [x] **测试环境**: 集成 Jest + SQLite (In-Memory) 用于单元测试
-   [ ] **其他模块迁移**: Finance, Generate 等模块待从 PyApp/NextJS API 迁移
