...
import { containsEmoji, createHtmlWrapper, getResizeOptions } from './algo'
import sharp from 'sharp'
import { APP_CONSTANTS, ERROR_MESSAGES } from '../constants'
import { Logger } from '@nestjs/common'

const logger = new Logger('ImageConverter')
...
export async function convertSvgToPng(svgContent: string): Promise<Buffer> {
    ...
    try {
        if (needEmojiSupport) {
            logger.log('使用 Playwright 渲染以支持 emoji')
        ...
        logger.log('Sharp 直接渲染完成')

        return png
    } catch (error) {
        logger.warn('Playwright渲染失败，尝试Sharp回退', error)
        try {
            const buffer = Buffer.from(svgContent)
            const png = await withSharpTimeout(
                sharp(buffer)
            ...
            logger.log('Sharp回退渲染成功')

            return png
        } catch (fallbackError) {
            logger.error('Sharp回退渲染失败', fallbackError)
            throw new Error(
...
export async function convertBase64ToPng(base64Data: string): Promise<Buffer | undefined> {
    ...
    logger.log('开始处理Base64数据', { length: base64Data.length })
    try {
        ...
        logger.error('Base64转PNG最终错误', error)
        throw new Error(
            `${ERROR_MESSAGES.IMAGE_CONVERSION_FAILED}: ${error instanceof Error ? error.message : ERROR_MESSAGES.UNKNOWN_ERROR}`,
        )
    }
...
function prepareBase64Buffer(base64Data: string): Buffer {
    ...
    logger.log('Base64转换完成', { bufferSize: imageBuffer.length })
...
function createSharpProcessMethods(imageBuffer: Buffer): Array<() => Promise<Buffer>> {
    return IMAGE_CONVERTER_CONFIG.SHARP_METHODS.map((method: SharpMethod, index) => {
        return async (): Promise<Buffer> => {
            logger.debug(`尝试方法${index + 1}: ${method.name}`)
...
async function tryProcessMethods(methods: Array<() => Promise<Buffer>>): Promise<Buffer> {
    ...
    for (const fn of methods) {
        try {
            ...
            logger.log(`方法${idx + 1}成功`, { outputBytes: result.length })

            return result
        } catch (methodError) {
            logger.error(`方法${idx + 1}失败`, methodError)
